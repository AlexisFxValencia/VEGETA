class cutManager{
	...
	set_sliders_boundaries(){
		let x_slider = document.getElementById("x_slider");
	...
	set_planes_position(){
		...
		this.z_plane.constant = this.scene_manager.bbox.max.z + 1;
		
		//helper = boite filaire d'aide qui englobe tous les mesh de la scene.
		//let helper = new THREE.Box3Helper(bbox, new THREE.Color(0, 255, 0));
		//scene_manager.scene.add(helper);	
		this.set_sliders_boundaries();
		
		let that = this;

		let x_slider = document.getElementById("x_slider");
		let x_writing_zone = document.getElementById("x_writing_zone");
		...
		let y_slider = document.getElementById("y_slider");
		let y_writing_zone = document.getElementById("y_writing_zone");
		// Update the current slider value (each time you drag the slider handle)
		...
		let z_slider = document.getElementById("z_slider");
		let z_writing_zone = document.getElementById("z_writing_zone");
		// Update the current slider value (each time you drag the slider handle)
